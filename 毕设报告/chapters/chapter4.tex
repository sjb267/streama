...
\begin{figure}[htbp]
    ...
    $\Downarrow$\\[0.4em]
    MySQL \quad Redis \quad Elasticsearch \quad RabbitMQ \quad Seata \quad 文件存储\\[0.4em]
    $\Updownarrow$\\[0.4em]
    AI 审核服务
    \end{minipage}
    }
...
\begin{table}[htbp]
    ...
    数据支撑层 & MySQL、Redis、Elasticsearch、RabbitMQ、Seata、文件目录 & 保存业务数据、缓存、搜索索引、消息、事务状态和媒体资源 \\
    \hline
    \end{tabular}
\end{table}
...
\section{事务与一致性设计}
\label{sec:transaction-consistency-design}

在线视频平台的业务链路中既存在单服务本地写操作，也存在跨服务协作写操作。例如，用户投稿、审核状态流转、互动计数变化和投币行为都可能同时涉及主站服务、资源服务、互动服务或后台管理服务。若某一环节调用失败，可能出现部分数据已提交而其他数据未更新的问题，因此系统需要在总体设计阶段区分强一致和最终一致两类场景。

对于关键跨服务写操作，系统引入 Seata AT 模式作为分布式事务支撑。Seata 通过事务协调器管理全局事务，并结合业务数据库中的 \texttt{undo\_log} 记录完成异常回滚支持\cite{seatadocs}。在本系统中，Seata 不作为独立业务功能展示，而是作为跨服务写操作的数据一致性保障机制。

对于视频转码完成回写、播放计数累加、搜索索引同步、文件删除等不要求立即强一致的场景，系统采用异步队列、状态字段和补偿处理方式实现最终一致。这样既避免所有业务都依赖强事务导致系统复杂度上升，也能保证核心写操作具有较好的可靠性。
